...
async function options(_, res) {
    try {
        const data = await perfil.options();
        return sendSuccess(res, data);
    } catch (error) {
        return sendError(res, "Se produjo un error de servidor, intente nuevamente.", "Perfil/options", error);
    }
}
